...
async def test_create_version(
    ...
    page_id = client.create_page(name="test", title="Test Page")
    assert page_id == "fakeid"
...
def test_create_version_missing_header(client: HTTPTestClient) -> None:
    """Check response from POST /pages/{page_id}/versions/."""
    ...
    assert version_create_response.json() == {
        "error": "x-page-version header must be present"
    }
