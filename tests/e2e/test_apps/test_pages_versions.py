...
async def test_create_page_and_publish_version_then_expect_pages(
    client: InMemoryHTTPPagesClient, event_bus: EventBus
) -> None:
    ...
    created_waiter = await Waiter.create(event_bus.subscribe(PAGE_VERSION_CREATED))
    uploaded_waiter = await Waiter.create(event_bus.subscribe(PAGE_VERSION_UPLOADED))
